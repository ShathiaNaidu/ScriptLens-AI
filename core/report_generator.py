...
def generate_docx(report: AnalysisReport) -> bytes:
    ...
    document.add_heading("Target Market of the ScriptLens App", level=1)
...
def _register_pdf_font() -> str:
    ...
    for path in candidates:
        if path.exists():
            try:
                pdfmetrics.registerFont(TTFont("ScriptLensFont", str(path)))
                return "ScriptLensFont"
            except Exception:
                continue
    return "Helvetica"
...
def generate_pdf(report: AnalysisReport) -> bytes:
    ...
    story.append(Paragraph("Target Market of the ScriptLens App", styles["SLHeading1"]))
